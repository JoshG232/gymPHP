...
<?php
  ...
  if (isset($_POST["submit"])) {
    ...
    if (empty($nameErr) && empty($setNumErr) && empty($weightErr) && empty($repsErr)){
      //Add to the database
      $sql = "INSERT INTO legs2 (name,setNum,weight,reps) VALUES 
      ('$name','$setNum','$weight','$reps')";

      if (mysqli_query($conn, $sql)){
        header('Location: /gymV1/legs2.php');
    ...
    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" class="mt-4 w-75" method="POST">
      <div class="mb-3">
        ...
        <select class="form-control" id="name" name="name">
          ...
          <option value="Leg curls/Ham string curls">Leg curls/Ham string curls</option>
      ...
      <div class="mb-3">
        ...
        <textarea class="form-control <?php echo $repsErr ? "is-invalid" : null; ?>" id="reps" name="reps" placeholder="Enter your reps"></textarea>
        <div class="invalid-feedback">
          <?php echo $repsErr; ?>
        </div>
      </div>
